...
const Pagination = ({ OnChangePage, size }) => {
...
const MobileNavigation = ({ isOpen, closeNav }) => {
  return (
    <AnimatePresence>
      <motion.div
        initial={{
...
const AllProducts = ({ products }) => {
  ...
  const [pagItemsNumber, setPagItemsNumber] = useState([0, PAG_NUMBER])
  ...
  const handleChange = (e) => {
    ...
    //   setShowedProducts(showed => showed.sorted((a, b) => a.price - b.price))
    // } else if (e.target.value === 'alphabet') {
    //   showedProducts.sort((a, b) => a.title.localeCompare(b.title))
    // }
  }
  ...
  const pageNumberHandler = (start, finish) => {
  ...
  return (
    <div className='second flex-grow mb-20 relative'>
      ...
      {/* {!filterIsOpen && (
        <button
          className='ml-5 text-2xl font-bold mb-5 transition-colors 
        hover:text-main duration-200 px-4'
          onClick={() => setFilterIsOpen(true)}
        >
          Filter
        </button>
      )} */}
      {/* {filterIsOpen && <MobileNavigation filterProducts={setFilteredProducts} products={products}/>} */}
...
const Shop = () => {
  let products = useSelector((state) => state.prods.products)
